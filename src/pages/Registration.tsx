...
import {createNewUserAction} from "../redux/profileReducer/actions";

export const Registration = () => {
    const dispatch = useDispatch()
    const { register, handleSubmit } = useForm();

    const onSubmit = (data: FieldValues) => dispatch(createNewUserAction(data.email, data.name, data.password))
            ...
            <div className="rounded-3xl bg-white text-center py-4 px-8">
                ...
                <form className="gap-4 grid" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="flex justify-between">
                        ...
                        <button type='submit' className="w-2/5 bg-auth-bg-button font-semibold uppercase text-white py-4 min-h-[50px] rounded-lg">
                            Create
                        </button>
                    </div>
                </form>
